...
export default function PosApp() {
  ...
  function addToCart(productId: number) {
    ...
    setCart((current) => {
      ...
      return current.map((i) => i.productId === productId ? { ...i, quantity: i.quantity + 1 } : i);
    });
  }
  ...
  function changeQty(productId: number, delta: number) {
    ...
    setCart((current) => current
      .map((i) => i.productId === productId ? { ...i, quantity: Math.min(max, Math.max(0, i.quantity + delta)) } : i)
  ...
  async function checkout() {
    ...
    try {
      ...
      const tx = await posApi(editing
        ? { action: 'update-transaction', receiptNo: editing.id, items: cart, payment, note }
        : { action: 'checkout', items: cart, payment, note });
      await loadData();
      setReceipt(tx);
      resetSale();
    } catch (e) {
  ...
  async function deleteTransaction(tx: Transaction) {
    if (!confirm(`Padam transaksi ${tx.id}? Stok akan dipulangkan.`)) return;
    try {
      setSaving(true); setError('');
      await posApi({ action: 'delete-transaction', receiptNo: tx.id });
      await loadData();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal padam transaksi.');
    } finally { setSaving(false); }
  }
  ...
  async function addProduct() {
    ...
    try {
      setSaving(true); setError('');
      await posApi({ action: 'product', ...newProduct });
      await loadData();
      setNewProduct({ name: '', sku: '', price: 0, cost: 0, stock: 0, lowStock: 5 });
      setAddProductOpen(false);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal tambah produk.');
    } finally { setSaving(false); }
  }

  async function stockIn() {
    if (!stockOpen || stockQty <= 0) return;
    try {
      setSaving(true); setError('');
      await posApi({ action: 'stock-in', productId: stockOpen.id, quantity: stockQty, note: stockNote });
      await loadData();
      setStockOpen(null); setStockQty(1); setStockNote('Restock');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Stock In gagal.');
    } finally { setSaving(false); }
  }

  const title = view === 'dashboard' ? 'Dashboard' : view === 'pos' ? 'Cash Register' : view === 'inventory' ? 'Inventory' : 'Daily Reports';

  return (
    <main className="posAppShell">
      <aside className="posSidebar">
        <div className="posBrand"><div className="posBrandMark"><Sparkles size={20} /></div><div><b>Squishy POS</b><span>Sales & Inventory</span></div></div>
        <nav>
          <button className={view === 'dashboard' ? 'active' : ''} onClick={() => setView('dashboard')}><LayoutDashboard size={18} />Dashboard</button>
          <button className={view === 'pos' ? 'active' : ''} onClick={() => setView('pos')}><ShoppingBag size={18} />Sales / POS</button>
          <button className={view === 'inventory' ? 'active' : ''} onClick={() => setView('inventory')}><Package size={18} />Inventory</button>
          <button className={view === 'reports' ? 'active' : ''} onClick={() => setView('reports')}><BarChart3 size={18} />Daily Reports</button>
        </nav>
      ...
      <section className="posMainArea">
        <header className="posTopbar"><div><p>Squishy retail workspace</p><h1>{title}</h1></div><button className="posSecondaryButton" onClick={loadData}><RefreshCw size={16} />Refresh</button></header>
