...
export default function PosApp({ currentUser }: { currentUser: SessionUser }) {
  const isAdmin = currentUser.role === 'admin';
  const [view, setView] = useState<View>(isAdmin ? 'dashboard' : 'pos');
  ...
  async function saveProductEdit() {
    if (!isAdmin || !editProductOpen || !editProductValue.name.trim() || !editProductValue.sku.trim() || editProductValue.price <= 0) return;
    try {
      setSaving(true); setError('');
  ...
  return <main className="posAppShell">
    <aside className="posSidebar"><div className="posBrand"><div className="posBrandMark"><Sparkles size={20} /></div><div><b>Squishy Toy POS</b><span>Play • Sell • Smile</span></div></div><nav>{isAdmin && <button className={view === 'dashboard' ? 'active' : ''} onClick={() => setView('dashboard')}><LayoutDashboard size={18} />Dashboard</button>}<button className={view === 'pos' ? 'active' : ''} onClick={() => setView('pos')}><ShoppingBag size={18} />Sales / POS</button>{isAdmin && <button className={view === 'inventory' ? 'active' : ''} onClick={() => setView('inventory')}><Package size={18} />Inventory</button>}{isAdmin && <button className={view === 'reports' ? 'active' : ''} onClick={() => setView('reports')}><BarChart3 size={18} />Daily Reports</button>}</nav><div className="posDbStatus"><span className={loading ? 'posDot busy' : 'posDot'} />{loading ? 'Connecting...' : `${currentUser.role === 'admin' ? 'Admin' : 'Cashier'} • Neon live`}</div></aside>
    <section className="posMainArea">
      <header className="posTopbar"><div><p>{currentUser.role === 'admin' ? 'Admin workspace' : 'Cashier workspace'} ✨</p><h1>{title}</h1></div><button className="posSecondaryButton" onClick={loadData}><RefreshCw size={16} />Refresh</button></header>
      {error && <div className="posAlert"><span>{error}</span><button onClick={() => setError('')}>×</button></div>}
      {view === 'dashboard' && isAdmin && <DashboardView products={activeProducts} transactions={transactions} onNewSale={() => setView('pos')} />}
      ...
      {view === 'inventory' && isAdmin && <InventoryView products={activeProducts} onAddProduct={() => setAddProductOpen(true)} onEditProduct={openProductEditor} onStockIn={(product) => { setStockOpen(product); setStockQty(1); setStockNote('Restock'); }} onDeleteProduct={archiveProduct} />}
      {view === 'reports' && isAdmin && <ReportsView products={products} transactions={transactions} query={query} dateFilter={dateFilter} onQuery={setQuery} onDateFilter={setDateFilter} onExport={(rows) => exportTransactionsCsv(rows, productMap)} onView={(tx) => { setReceiptCash(null); setReceipt(tx); }} onEdit={editTransaction} onDelete={deleteTransaction} />}
    </section>
    {isAdmin && addProductOpen && <AddProductModal value={newProduct} saving={saving} onChange={setNewProduct} onClose={() => setAddProductOpen(false)} onSave={addProduct} />}
    {isAdmin && editProductOpen && <EditProductModal product={editProductOpen} value={editProductValue} saving={saving} onChange={setEditProductValue} onClose={() => setEditProductOpen(null)} onSave={saveProductEdit} />}
    {isAdmin && stockOpen && <StockInModal product={stockOpen} quantity={stockQty} note={stockNote} saving={saving} onQuantity={setStockQty} onNote={setStockNote} onClose={() => setStockOpen(null)} onSave={stockIn} />}
