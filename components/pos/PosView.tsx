...
export default function PosView({ products, cart, payment, note, editing, saving, productQuery, discountType, discountValue, cashReceived, onProductQuery, onAdd, onQty, onRemove, onPayment, onNote, onDiscountType, onDiscountValue, onCashReceived, onCheckout, onCancelEdit }: {
  ...
  return <section className="posCheckoutLayout posV2Layout">
    ...
    <div className="posCard posCartPanel posV2Cart">
      ...
      {payment === 'Tunai' && <div className="posCashBox"><div className="posCashHeader"><span>Cash Received</span><b>{money(cashReceived)}</b></div><div className="posQuickCash">{quickCash.map((value) => <button key={value} onClick={() => onCashReceived(value)}>RM{value}</button>)}<button onClick={() => onCashReceived(Math.ceil(total))}>Exact</button></div><input type="number" min="0" step="0.01" value={cashReceived || ''} placeholder="Enter cash received" onChange={(e) => onCashReceived(Math.max(0, Number(e.target.value) || 0))} /><div className={`posChangeDue ${cashShort ? 'short' : ''}`}><span>{cashShort ? 'Cash short' : 'Change Due'}</span><strong>{cashShort ? money(total - cashReceived) : money(changeDue)}</strong></div></div>}
