...
export function EditProductModal({ product, value, saving, onChange, onClose, onSave }: {
  product: Product;
  value: ProductFormValue;
  saving: boolean;
  onChange: (value: ProductFormValue) => void;
  ...
  return <Modal title={`Edit Product — ${product.name}`} onClose={onClose}>
    <div className="posFormGrid">
      <label>Name<input value={value.name} onChange={(e) => onChange({ ...value, name: e.target.value })} /></label>
      <label>SKU<input value={value.sku} onChange={(e) => onChange({ ...value, sku: e.target.value })} /></label>
      <label>Price (RM)<input type="number" min="0" step="0.01" value={value.price} onChange={(e) => onChange({ ...value, price: Number(e.target.value) })} /></label>
      <label>Cost (RM)<input type="number" min="0" step="0.01" value={value.cost} onChange={(e) => onChange({ ...value, cost: Number(e.target.value) })} /></label>
      <label>Current Stock<input type="number" min="0" step="1" value={value.stock} onChange={(e) => onChange({ ...value, stock: Math.max(0, Math.trunc(Number(e.target.value) || 0)) })} /></label>
    ...
    <p style={{ marginTop: 12, fontSize: 12, opacity: .7 }}>Changing Current Stock creates an inventory adjustment record automatically.</p>
    <div className="posModalActions"><button className="posSecondaryButton" onClick={onClose}>Cancel</button><button className="posPrimaryButton" disabled={saving || !value.name.trim() || !value.sku.trim() || value.price <= 0} onClick={onSave}>{saving ? 'Saving...' : 'Save Changes'}</button></div>
  </Modal>;
}
